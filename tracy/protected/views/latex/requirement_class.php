...
function requirement_row($req,$indentation,TableCreator $table_creator,$filter_cat,$filter_pri)
{
    ...
    if (  ( !is_numeric($filter_cat) || $req->category == $filter_cat ) &&
          ...
          isset($req->classes) && count($req->classes) > 0*/
        )
    {
		if (count($req->classes)) {
			$pkg_name = array();
    ...
    $nested = Requirement::model()->with('classes')
...
$toplevel = Requirement::model()->with('classes')
                    ->findAll("t.parent is null");

    
$table =
    $table_creator->begin_table("|r l|p{10cm}|") .
    $table_creator->heading_row(array("Requisito"=>2,"Classi"));
...
$fc = is_numeric($filter_cat) ?
        ...
        : "";
$table .= $table_creator->caption("Tabella requisiti $fc/ classi").
